...
import love.forte.simbot.quantcat.AnnotationEventInterceptorFactory
import kotlin.reflect.KClass

/**
 * 配合 [@Listener][Listener] 使用，为被标记的事件处理器添加一个目标工厂所产生的拦截器。
 *
 * [factory] 提供一个 [AnnotationEventInterceptorFactory] 的 **实现类型**。如果此类型是 `object`，
...
public annotation class Interceptor(
     ...
     * 则直接使用，否则会构建一个实例。此实例 **可能会被共享**，因此请考虑处理并发。
     *
     * 只有**第一个**元素有效。如果提供多个元素则会抛出异常。[Array] 类型仅为了提供默认值。
     * 如果希望添加多个拦截器，请使用多个 [Interceptor] 注解。
     *
     * [factory] 与 [factoryClassName] 同时存在时，优先使用 [factory]。
     */
    val factory: Array<KClass<out AnnotationEventInterceptorFactory>> = [],

    /**
     * 提供一个 [AnnotationEventInterceptorFactory] 的 **实现类型** 的全限定类名。如果此类型是 `object`，
     * 则直接使用，否则会构建一个实例。此实例 **可能会被共享**，因此请考虑处理并发。
     *
     * [factory] 与 [factoryClassName] 同时存在时，优先使用 [factory]。
     */
